...
import {
  ...
  setTodo,
  setTypeTodo,
} from "@/lib/features/trelloAPI";
import update from "immutability-helper";
import { Card } from "../Card/Card";
import { store } from "@/lib/store";
import { useDrop } from "react-dnd";
import { getTotalTypeTrello } from "@/app/util/trelloTotal";
...
export const Counter = () => {
  ...
  const handleCloseCofirm = () => {
    dispatch(deleteTodo({ id: idDelete } as any));
    setOpenConfirm(false);
    handleClose();
  };
  ...
  const renderCard = useCallback(
    (
      card: { id: number; name: string; type: number },
      index: number,
      columnIndex: number
    ) => {
      return (
  ...
  const DroppableArea: React.FC<any> = ({ onDrop }) => {
    ...
    return (
      <div
        ref={drop}
        style={{
          backgroundColor: isOver ? "rgba(255, 255, 255, 0.226)" : "black",
          height: "45px",
        }}
      ></div>
    );
  ...
  const handleDrop = (type: number, item: any) => {
    ...
    dispatch(
      setTypeTodo({ id: Number(item.id), type: type } as TrelloSliceState)
    );
    ...
    <>
      ...
      >
        <Box className="app__popup--container">
          <h1>
            {" "}
            Edit task "{edit.name}" - {edit.type == 1 && <span>Todo</span>}{" "}
            {edit.type == 2 && <span>Doing</span>}{" "}
            {edit.type == 3 && <span>Done</span>}
          </h1>
          <div className="app__popup--content">
            ...
            >
              <div className="app__popup--content-form-item">
                ...
                />
                {errorsUpdate?.name && (
                  <span>{errorsUpdate?.name?.message}</span>
                )}
              </div>
              <div className="app__popup--content-form-item">
                <TextareaAutosize
                ...
                {errorsUpdate?.content && (
                  <span>{errorsUpdate?.content?.message}</span>
                )}
              </div>
              ...
              <div className="app__popup--content-form-item">
                ...
                {errorsUpdate?.date && (
                  <span>{errorsUpdate?.date?.message}</span>
                )}
              </div>
            ...
            <div className="col-sm-4">
              <div className="app__todo">
                ...
                <div className="app__todo--content">
                  {getTotalTypeTrello(tasks, 1) == 0 && (
                    <DroppableArea
                      onDrop={(item: { id: number; index: number }) =>
                        handleDrop(1, item)
                      }
                    />
                  )}
                ...
                <div className="app__todo--content-form">
                  ...
                  {formShow.isShow && formShow.tab == "todo" && (
            ...
            <div className="col-sm-4">
              <div className="app__todo">
                ...
                <div className="app__todo--content-form">
                  ...
                  {formShow.isShow && formShow.tab == "doing" && (
                    <div className="app__todo--content-form-btns">
                      <form
                        onSubmit={handleSubmit(onSubmit)}
                        className="app__todo--content-form-add"
            ...
            <div className="col-sm-4">
              <div className="app__todo">
                ...
                <div className="app__todo--content-form">
                  ...
                  {formShow.isShow && formShow.tab == "done" && (
                    <div className="app__todo--content-form-btns">
                      <form
                        onSubmit={handleSubmit(onSubmit)}
                        className="app__todo--content-form-add"
